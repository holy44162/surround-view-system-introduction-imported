...
def main(save_weights=False):
    ...
    # added by Holy 2006061032
    strWeightPathName = "d:/backup/project/surround-view-system-introduction-imported/yaml/weights.yaml"
    with open(strWeightPathName, "r") as f:
        dataWeights = yaml.load(f, Loader=yaml.SafeLoader)

    weights[0] = np.array(dataWeights["G0"])
    weights[1] = np.array(dataWeights["G1"])
    weights[2] = np.array(dataWeights["G2"])
    weights[3] = np.array(dataWeights["G3"])
    # end of addition 2006061032
